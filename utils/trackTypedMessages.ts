...
export const trackSpamFromUser = async (
  userId: number,
  TIME_FRAME: number,
  MAX_MESSAGES: number,
  session: UserSession,
  ...
  userActivity.lastMessageTime = userActivity.lastMessageTime.filter(
    (time) => now - time <= TIME_FRAME,
  );
  ...
  if (userActivity.lastMessageTime.length > MAX_MESSAGES) {
    ...
    await sendMessageToUser(
      userId,
      "Ви були автоматично заблоковані за спам повідомленнями. Якщо у вас виникли питання, звертайтеся до @polter01",
    );
  }
};
