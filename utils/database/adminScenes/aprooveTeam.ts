...
export const aprooveTeam = async (query: any, teamId: string) => {
  ...
  await editInlineKeyboard(query, "Команда затверджена", [
    [{ text: "parada", callback_data: "parada" }],
  ]);
  await sendMessageByTeamId(
    teamId,
    "Вітаю! Ви прийшли тестове завдання! Перезапустіть бота командою /start, щоб дізнатися деталі проведення змагань",
  );
};
