    ...
    createInlineKeyboard(
      ...
      [
        ...
        [
          {
            text: "Розблокувати",
            callback_data: `unblock_user_${user.chat_id}`,
          },
        ],
  ...
  currentUsersListener = async (query: CallbackQuery) => {
    if (query.message?.chat.id === chatId) {
      ...
      if (action === "block") {
        ...
        userSessions.set(parseInt(userId), {
        ...
        await sendMessageToUser(
          parseInt(userId),
          "Ви були заблоковані адміністратором. Щоб продовжити роботу з ботом, зверніться до адміністратора. @polter01",
        );
        ...
        await sendMessageToUser(
          parseInt(userId),
          "Ви були розблоковані адміністратором.",
        );
...
async function deleteUser(userId: number) {
  ...
  await sendMessageToUser(
    userId,
    "Ваш акаунт було видалено адміністратором. Щоб продовжити роботу з ботом, використайте команду /start.",
  );
}
