  ...
  try {
    ...
    for (const team of teams) {
      await createInlineKeyboard(
        chatId,
        `ім'я команди: ${team.name} \nЧи апрувнута команда: ${
          team.isTestValid ? "так" : "ні"
        }`,
        [
          [{ text: "Видалити", callback_data: `delete-team_${team.tid}` }],
          [
            {
              text: "Тестове пройдено ✅",
              callback_data: `aproove-team_${team.tid}`,
            },
          ],
          [
            {
              text: "Тестове не пройдено ❌",
              callback_data: `decline-team_${team.tid}`,
            },
          ],
