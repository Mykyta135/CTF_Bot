...
export const teamAboutScene = async (
  chatId: number,
  query: CallbackQuery,
  teamId: any,
) => {
  const teamAboutString = await teamInfo(teamId);
  await editInlineKeyboard(query, teamAboutString, teamAboutLayout);
    ...
    if (query.message?.chat.id === chatId) {
      if (query.data === "back") {
        await editInlineKeyboard(
          query,
          startMessage,
          await handleKeyboardLayout(chatId),
        );
      }
      if (query.data === "delete-the-team") {
        await deleteTeam(query, teamId, [[]]);
        await sendMessageByTeamId(
          teamId,
          "Ваша команда була видалена. Ви можете створити нову команду або приєднатися до іншої. Скористуйтесь командою /start, заново почати роботу",
        );
