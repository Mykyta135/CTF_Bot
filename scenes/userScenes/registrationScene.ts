...
export const RegistrationScene = async (
  chatId: number,
  session: UserSession,
) => {
  ...
  const userAge = await handleUserInput(
    session,
    chatId,
    "Введіть вік",
    age,
  ...
  const userUniversity = await handleUserInput(
    session,
    chatId,
    "Введіть університет",
    university,
  ...
  const userGroup = await handleUserInput(
    session,
    chatId,
    "Введіть групу",
    group,
  ...
  const userCourse = await handleUserInput(
    session,
    chatId,
    "Введіть курс",
    course,
  ...
  const userSource = await handleUserInput(
    session,
    chatId,
  ...
  const userContact = await handleUserContact(
    session,
    chatId,
    "Введіть контакт",
    contact,
      ...
      if (message.chat.id === chatId) {
        ...
        if (message.contact) {
          ...
          if (await safeParse(chatId, schema, input)) {
            ...
            bot.removeListener("message", contactHandler);
            resolve(
              await handleUserContact(session, chatId, promptText, schema),
            );
          }
        } else {
          await bot.sendMessage({
