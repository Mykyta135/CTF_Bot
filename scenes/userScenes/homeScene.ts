...
import {
  unregisteredHomeLayout,
  registeredHomeLayout,
  onEventHomeLayout,
} from "../../utils/keyboards/homeSceneLayout";
...
export const HomeScene = async (chatId: number) => {
  ...
  const currentMessage = await createInlineKeyboard(
    chatId,
    startMessage,
    keyboardLayout,
  );
  ...
  handleCallbackQuery = async (query: CallbackQuery) => {
    ...
    if (
      ...
      query.message.message_id === lastInlineMessageId.get(chatId)
    ) {
...
const handleRegistrationScene = async (chatId: number, query: any) => {
  ...
  if (session.userState === "unregistered") {
    ...
    await editInlineKeyboard(
      query,
      "Ви вже знаходитесь на етапі реєстрації. Якщо хочете вийти, або почати спочатку, використайте команду /start",
      [],
    );
  } else {
...
export const removeUnneceseryListeners = (
  handleCallbackQuery: any,
  currentAdminListener: any,
) => {
